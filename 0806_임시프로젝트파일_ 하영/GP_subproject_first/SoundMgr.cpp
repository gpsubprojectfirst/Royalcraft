...
HRESULT SoundMgr::Init(void)
{
	...
	{
		MessageBox(AfxGetMainWnd()->GetSafeHwnd(), L"사운드디바이스생성실패", L"SYSTEM ERROR", MB_OK);
		return E_FAIL;
	}
	...
	{
		MessageBox(AfxGetMainWnd()->GetSafeHwnd(), L"사운드디바이스 협조레벨 설정", L"SYSTEM ERROR", MB_OK);
		return E_FAIL;
	}
...
HRESULT SoundMgr::LoadWave(TCHAR* pFileName)
{
	...
	if (FAILED(m_pSound->CreateSoundBuffer(&BuffInfo, &SoundBuff
		, NULL)))
	{
		MessageBox(AfxGetMainWnd()->GetSafeHwnd(), L"사운드버퍼생성실패", L"", MB_OK);
		return E_FAIL;
	}
